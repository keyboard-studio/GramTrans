...
The PyQt UI layer (`src/gramtrans/ui/`) MUST NOT import from `flavors/` or
`categories/`. Its only window onto the engine is the small surface declared here.
...
|-----------|-------|---------------------|-----------|
| `main_window.py` | `RunContext`, `Selection`, current `RunPlan`, current `RunReport` | `Selection`, triggers `compute_preview` / `execute_move` | Direct LCM access, importing `flavors/` or `categories/`. |
