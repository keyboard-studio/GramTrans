...
- [x] T022 Implement `src/gramtrans/gramtrans.py` per the FLExTrans convention: module-level `docs = {FTM_Name, FTM_Version, FTM_ModifiesDB, FTM_Synopsis, FTM_Help, FTM_Description}` dict + `MainFunction(project, report, modifyAllowed)` callable, with `import site; site.addsitedir(r"Lib")` at the top. `MainFunction` instantiates the PyQt main window. This **replaces** the v4.0.0 `module.py` + `FlexToolsModuleClass` plan; STATUS.md confirms the no-wrapper template is what the MCP returns.
...
- [x] T066 [US2] Extend `src/gramtrans/Lib/ui/stats_panel.py` to render the skip list with reasons + the identity remap section per the ASCII layout in [contracts/run-report.md](contracts/run-report.md)
