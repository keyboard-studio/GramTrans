...
- [X] T005 Create `specs/007-affixes-stems/probe-results.md` skeleton (sections per R1–R11 of research.md); subsequent probe tasks append to it
- [X] T006 [P] MCP-probe `ILexExampleSentenceFactory`, `ILexPronunciationFactory`, `ILexEtymologyFactory`, `ILexEntryRefFactory` — `ILexExampleSentenceFactory.Create(Guid, ILexSense)` Guid-preserved; other 3 are 0-method stubs → ServiceLocator fallback
- [X] T007 [P] MCP-probe MSA subclass operations — all 4 wrappers found in `MSAOperations` (`CreateInflAff(sense, pos, slots=None)`, `CreateDerivAff(sense, from_pos, to_pos)`, `CreateStem(sense, pos)`, `CreateUnclassifiedAffix(sense, pos)` — note: name is `CreateUnclassifiedAffix`, not `CreateUnclassified`)
- [X] T008 [P] MCP-probe Endo/Exo compound factories + accessors — both factories are 0-method stubs (ServiceLocator); **major correction**: NO `LeftMsaOA`/`RightMsaOA` exist on the rule. Endo adds `HeadLast` + `OverridingMsaOA` (owned `IMoStemMsa`); Exo adds `ToMsaOA` (owned `IMoStemMsa`); base `IMoCompoundRule` carries `Description` (Carrier B residue), `Disabled`, `Name`, `StratumRA`, `ToProdRestrictRC`. C3 (residue carrier) closed: Carrier B on `Description`.
- [X] T009 [P] MCP-probe Ad-hoc subclasses — **major correction**: 3-way split, not 2-way. `MoAdhocProhibGr` (group, owns `MembersOC` — NOT `MembersRS` — carries `Description` Carrier B), `MoAlloAdhocProhib` (atomic, allomorph-based, `AllomorphsRS`+`FirstAllomorphRA`+`RestOfAllosRS`, NO residue carrier), `MoMorphAdhocProhib` (atomic, morpheme-based, parallel shape on `MorphemesRS`, NO residue carrier). No `IMoAdhocProhibAtomFactory` exists.
- [X] T010 [P] MCP-probe `IMoInflAffixTemplate` — **5 slot ref sequences**: `PrefixSlotsRS`, `SuffixSlotsRS`, `EncliticSlotsRS`, `ProcliticSlotsRS`, `SlotsRS` (spec assumed 2). Plus `Final` (bool), `RegionOA`, `StratumRA`. `IPartOfSpeech.AffixSlotsOC` (OC) + `AffixTemplatesOS` (OS) confirmed.
- [X] T011 [P] MCP-probe `IMoMorphData` accessors — `AdhocCoProhibitionsOC` (owned collection, `.Add`) + `CompoundRulesOS` (owned sequence, `.Append`) + `StrataOS` confirmed.
- [X] T012 [P] MCP-probe Ejagham Mini inventory — **252 entries (88 affix + 164 stem), 293 allomorphs, 247 MSAs (83 InflAff + 164 Stem only; no Deriv/Unclassified), 0 compound rules, 0 ad-hoc prohibitions, 7 templates, 9 slots across 20 POSes (6 with slots, 6 with templates), 6 EntryRefs with components, 0 with primary**. **STATUS.md's "13 verb-affix entries" was the Verb-only subset; full affix count is 88.**

**Checkpoint**: T005–T012 complete; 18 spec corrections enumerated in `probe-results.md`. **US4 (compound + ad-hoc) live-verification path blocked** — Ejagham Mini has 0 of both; US4 live verification deferred (Option (a) per probe-results: synthetic fixtures only, document gap in verification-log.md). **MSA dispatch scope shrinks**: US1 MVP needs only `MoInflAffMsa` + `MoStemMsa` paths to ship a working Ejagham Mini transfer end-to-end; `MoDerivAffMsa`/`MoUnclassifiedAffixMsa` stay as `Skip(NEEDS_MANUAL)` stubs without breaking live verification.
