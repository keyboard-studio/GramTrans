...
**Constraint**: Custom field creation is a schema mutation that the LCM forbids inside an open UnitOfWork. GramTrans runs inside a Phase-1 transaction envelope (`BeginNonUndoableTask` at `OpenProject`, held until `CloseProject`). Creation is therefore blocked at the flexlibs2 layer; bypassing the guard with raw `AddCustomField` corrupts the project on next FLEx UI open (flexlibs2 issue #21). The adopted workaround is the FLEx UI pre-create workflow documented in `flexlibs2/docs/CUSTOM_FIELDS.md`. Scripted creation will be unblocked when Phase 2 transaction mode lands in flexlibs2. See [us2-blocker-memo.md](us2-blocker-memo.md).
