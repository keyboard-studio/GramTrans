...
### User Story 2 — Detect and report Custom Field definition gaps (Priority: P1)

A linguist's source project has FLEx custom-field definitions like "Noun class" on LexEntry, "Tone melody" on LexSense, "Loanword origin" on LexEntry. She runs Phase 3b with Custom Fields selected. GramTrans detects which of those definitions already exist in the target and which are absent, and emits a clear `Skip(NEEDS_MANUAL)` notice for each absent field directing her to pre-create it via Tools > Configure > Custom Fields before running Phase 3c. Without this pre-creation step, Phase 3c entries that carry "Noun class" values would silently drop those values on import.

**Why this priority**: P1 because Phase 3c (LexEntries with custom-field values) loses data silently when target fields are absent. The detect-and-report step surfaces the gap before Phase 3c runs, converting a silent-data-loss bug into a visible pre-flight checklist item.

**Constraint**: Custom field creation is a schema mutation that the LCM forbids inside an open UnitOfWork. GramTrans runs inside a Phase-1 transaction envelope (`BeginNonUndoableTask` at `OpenProject`, held until `CloseProject`). Creation is therefore blocked at the flexiconlayer; bypassing the guard with raw `AddCustomField` corrupts the project on next FLEx UI open (fflexiconssue #21). The adopted workaround is the FLEx UI pre-create workflow documented in `flflexiconcs/CUSTOM_FIELDS.md`. Scripted creation will be unblocked when Phase 2 transaction mode lands in fleflexicone [us2-blocker-memo.md](us2-blocker-memo.md).

**Independent Test**: Source with 2-3 custom fields (different classes, types). Target has none. Run Phase 3b Custom Fields. Verify each absent field produces one `Skip(NEEDS_MANUAL)` line in the report with the pre-create instruction. Re-run after manually creating the fields in FLEx UI: verify all fields now emit `Skip(ALREADY_PRESENT_BY_GUID)` and zero `NEEDS_MANUAL` lines remain.

**Acceptance Scenarios**:

1. **Given** source has "Noun class" on LexEntry and target has no custom fields, **When** the linguist transfers Custom Fields, **Then** the run report emits `[skip][NEEDS_MANUAL] custom_fields: "Noun class" (LexEntry) — Pre-create in FLEx > Tools > Configure > Custom Fields; names are case-sensitive` and no schema mutations occur.
...
3. **Given** source has "Noun class" on LexEntry and target already has "Noun class" on LexEntry, **When** the linguist transfers Custom Fields, **Then** the run report emits `Skip(ALREADY_PRESENT_BY_IDENTITY)` for that field and zero `NEEDS_MANUAL` notices.
...
- **FR-323**: System MUST implement the five callbacks (`enumerate_source`, `dependencies`, `required_writing_systems`, `plan_action`, `execute_action`) for `variant_types`, `complex_form_types`, and `semantic_domains` following the existing five-callback contract from `Lib/categories.py`. For `custom_fields`, `enumerate_source`, `dependencies`, `required_writing_systems`, and `plan_action` MUST be implemented; `execute_action` MUST be a registered no-op stub. `plan_action` emits `Skip` directly (`ALREADY_PRESENT_BY_GUID` when target has the field, `NEEDS_MANUAL` otherwise) so the executor path stays cold. Creation is blocked at the flexiconlayer (see [us2-blocker-memo.md](us2-blocker-memo.md)); the `execute_action` stub will be promoted to a live implementation when Phase 2 transaction mode lands in fflexicon
...
- **FR-325**: Custom-field transfer MUST detect existing target custom fields by `(class_id, name)` identity. For each source field found in target with matching `(class_id, name)`: emit `Skip(ALREADY_PRESENT_BY_IDENTITY)` (distinct from `ALREADY_PRESENT_BY_GUID` because custom fields have no LCM Guid; the match is a tuple lookup, not a Guid lookup). For each source field absent from target, emit `Skip(NEEDS_MANUAL)` with a detail line citing FLEx UI > Tools > Configure > Custom Fields and noting that names are case-sensitive. System MUST NOT attempt `CreateField` or `AddCustomField` inside the Phase-1 transaction envelope. Class-rebinding is never permitted: a field defined on LexEntry in source is never matched against a LexSense field of the same name in target.
...
- The flexiconfork at `D:/Github/_Projects/_LEX/fflexiconexposes Operations classes (or accessible factories) for all nine Phase 3b categories: `project.GramCat`, `project.InflectionFeatures` (with `InflectionClassGetAll`), `project.POS` (with `InflectionClassesOC` / `StemNamesOC` / `BearableFeaturesRC`), plus whatever surface flflexiconovides for CustomFields, VariantTypes, ComplexFormTypes, and SemanticDomains. The exact accessor names will be MCP-probed at planning time and recorded in `probe-results.md`.
...
- Constitution v5.0.0 Principle II (flexiconDirect) — all new code imports fflexiconirectly. No flavor-adapter shape introduced.
